...
module Controller =
    ...
    let showAction (ctx: HttpContext) (id: string) =
        task {
            ...
            | Ok None -> return! Response.notFound ctx "Value not found"
            | Error ex -> return raise ex
        }

    let createAction (ctx: HttpContext) =
        task {
            let! input = Controller.getModel<CreateRecipeDto> ctx
            let validateResult = Validation.validateCreateDto input
            if validateResult.IsEmpty then

                let cnf = Controller.getConfig ctx
                let! result = Database.insert cnf.connectionString input
                match result with
                | Ok _ -> return! Response.ok ctx ""
                | Error ex -> return raise ex
            else
    ...
    let updateAction (ctx: HttpContext) (id: string) =
        task {
            let! input = Controller.getModel<UpdateRecipeDto> ctx
            let validateResult = Validation.validateUpdateDto input
            if validateResult.IsEmpty then
                let cnf = Controller.getConfig ctx
